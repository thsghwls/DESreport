...
//			>>	8bit의 key 8개를 64bit key(여기선 32bit 2개)로 합치기 위해 수행 <<
	for( i = 0 ; i < 4 ; i++)
	{
//			>> 8bit 키 4개를 32bit로 만드려고 Shitf 사용 << [ 아래 연산을 수행하면 24, 16, 8 , 0 의 위치에 해당 key를 넣는다.(8bit차이) ]
		SHIFT =((3-i)*8);
//			>> Key 8개를 4개씩 32bit공간에 넣는다. XOR연산과 왼쪽 Shift연산을 사용해 32bit 변수 dwkey[1], dwkey[0]에 각각 넣는다.
		dwKey[1] ^= ((WORD)Key[i   ])<<SHIFT;		// Key[0]~Key[3]까지 dwKey[1]에 저장한다.
		dwKey[0] ^= ((WORD)Key[i+4])<<SHIFT;		// Key[4]~Key[7]까지 dwKey[0]에 저장한다.
																	// 여기서 Key는 8bit의 char형이므로 XOR연산을 위해 32bit WORD형으로 강제형변환을 한다.
	}

//	>> 순열선택1을 수행하여 64bit(32bit 2개)의 Key를 56bit(28bit 2개)의 Key로 축소 <<
...
//	>> 56bit key를 가지고 왼쪽 환형 시프트를 수행하고 48bit의 Key로 축소한다. 이를 16회 수행하여 RoundKey를 16개 만든다. <<
	for(i = 0 ; i < 16 ; i++)
	{
		...
		if(LSHIFT[i] == 1)
		{
			...
			//	Left 4bit 제외한 좌측 첫번째에 해당하는 dwKey[0]의 bit를 뺀 나머지 bit를 검출하여 좌측으로 한칸 옮기고 tmp0와 OR연산한다.
...
/*		순열선택2의 결과로 얻은 48bit의 i+1 라운드 키를 RoundKey[2*i+1]과 RoundKey[2*i]에 각각 24bit씩 저장.
		ex) i 가 0이면 1라운드 키를 RoundKey[1]과 RoundKey[0]에 각각 24bit씩 저장한다.							*/
