...
{
	int i;
	char key[8]		      	= {'x','z','e','w','h','k','k','y'};	//	64bit(8 x 8bit)의 key 준비
	WORD plainText[2] 	= {0x55555555 , 0x12312312};	//	64bit 평문 준비 [ Left(32bit) = plainText[1] , Right(32bit) = plainText[0] ]
	WORD roundKey[32] = {0};										//	각 라운드에 들어갈 48bit의 Round Key 준비
																			// [ key하나로 서로다른 16개의 Round key 준비, 24bit씩 들어가야 하므로 WORD 32개 선언 ]
