...
//	>> NewData 즉, 초기순열 이후의 plainText값을 우측부터 좌측순으로 넣기위해 사용 [ ex) 좌측<- 00000000 ...00000001 <-우측 ] <<
	WORD setMask;
//	>> plainText의 평문을 비교하며 초기순열을 만들어야하기 때문에 빈 공간인 NewData 생성 후 마지막에 plainText값을 변경하기위해 사용 <<
	WORD NewData[2]= {0};

/* >> 평문을 32bit씩 끊어서 Left와 Right로 나눴기 때문에 PI[0]~PI[31]과 PI[32]~PI[63]를 나눠 Initial Permutation 한다.
		 IP[i]의 4 x 8 (PI[0]~PI[31])에 해당하는 32bit를 검출하는데, IP[i]에 해당하는 위치에 있는(좌측기준, IP[i]가 32이면 Data의 0x00000001위치 라는의미 )
	...
	for(i = 31 ; i >= 0 ; i--)
	{
		tmpMask = 0x00000001; // 좌측기준으로 IP[i]의 숫자에 해당하는 위치의 비트를 가리킨다. 그 위치의 plainText가 1일경우 and연산으로 True가 된다.
		setMask = 0x00000001<<(31-i);	// NewData[1]의 우측부터 좌측으로 i만큼 이동하며 가리킨다. (만약 검출된 plainText bit가 1일경우 Set)
		if((IP[i] <= 32) && (Data[1] & (tmpMask << (32-IP[i])))) {	/* 좌측기준 IP[i]의 값에 해당하는 위치에 Data[1]의 값이 1이면(AND연산으로 확인)
																						그리고 IP[i]가 32이하면 NewData[1]의 위치(우측부터 좌측순으로 채워간다)에 1을 넣음 */
			NewData[1] = NewData[1] | setMask;
		}else if((IP[i] > 32) && (Data[0] & (tmpMask << (64-IP[i])))){
			NewData[1] = NewData[1] | setMask;
		}
//	 >> 위처럼 if , else if로 32이상과 32미만을 나누는 이유는 평문을 32bit씩 나누었기 때문이다. <<
	}

//		>> 위와 동일한데 이번에는 plainTextd에서 IP[]의 4 x 8 (PI[0]~PI[31])에 해당하는 위치에 있는 32bit를 검출하여 NewData[0]에 우측부터 넣는다. <<
	for(i = 63 ; i >=32 ; i--)
	{
		tmpMask = 0x00000001;
